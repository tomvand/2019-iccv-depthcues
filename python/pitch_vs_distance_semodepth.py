...
from numpy import sum, load, mean, zeros_like, average, array, std, float64
...
cfg = {
    ...
    'output_path': '/media/tom/seagate-8tb-ext4/data/kitti_derived/pitch/output_semodepth/pitch_vs_disp.csv',
    'Bf': 0.54 * 721.5377,
}
cfg['filenames_path'] = os.path.join(cfg['data_path'], 'filenames.txt')
cfg['disp_path'] = os.path.join(cfg['data_path'], 'output_semodepth')
...
with open(cfg['filenames_path'], 'rt') as f:
    for index, filename in enumerate(f):
        ...
        for i in obj_indices:
            ...
            disp = cfg['Bf'] / sio.loadmat(os.path.join(cfg['disp_path'], filename.replace('.png', '.mat')))['mat'][0, :, :, 0]
            disp_r = resize(disp, mask.shape)
            mask[sem == i] = 1.0
            mask[disp_r == 0] = 0.0
