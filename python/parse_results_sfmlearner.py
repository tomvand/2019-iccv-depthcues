...
cfg = {
    ...
    'disparities_path': '/media/tom/seagate-8tb-ext4/data/kitti_derived/pos_vs_scale/output_sfmlearner',
    'pickle_path': '/media/tom/seagate-8tb-ext4/data/kitti_derived/pos_vs_scale/output_sfmlearner/depths.p',
...
disparities_files = glob.glob(cfg['disparities_path'] + '/disparities_*.npy')
...
for i in range(len(disparities_files)):
    ...
    with open(filenames_files[i], 'rt') as f:
        for slice, filename in enumerate(f):
            ...
            depth = disparities[slice, :, :] # SfMLearner already outputs depth
