...
cfg = {
    ...
    'output_path': '/media/tom/seagate-8tb-ext4/data/kitti_derived/pitch/output_sfmlearner/pitch_vs_disp.csv',
}
cfg['filenames_path'] = os.path.join(cfg['data_path'], 'filenames.txt')
cfg['disp_path'] = os.path.join(cfg['data_path'], 'output_sfmlearner', 'disparities_.npy')
...
with open(cfg['filenames_path'], 'rt') as f:
    for index, filename in enumerate(f):
        ...
        for i in obj_indices:
            ...
            disp_r = resize(1.0 / disp[index], mask.shape) * mask.shape[1] # TODO disparity scale!
            mask[sem == i] = 1.0
            mask[disp_r == 0] = 0.0
